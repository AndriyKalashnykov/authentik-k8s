...
- `compose/forward-auth/` — optional overlay (`docker-compose.forward-auth.yml` + `traefik/dynamic.yml`) adding Traefik v3 + `traefik/whoami` to the base stack (exact tags pinned in `docker-compose.forward-auth.yml`, Renovate-tracked) to demo Authentik forward-auth. Brought up by `make compose-forward-auth-up` (which also configures the provider via the POC); verified by `make e2e-forward-auth` (curl) and `make e2e-forward-auth-browser` (Playwright). Both compose files are dclint-linted by `make compose-lint`.
- `e2e/browser/` — Playwright full-login browser e2e for the forward-auth demo (`forward-auth-login.spec.ts` + pinned `@playwright/test` in `package.json`/`package-lock.json`). Run via `make e2e-forward-auth-browser`; node is pinned in `provisioner/.mise.toml`. The only Node tooling in the repo; not part of build/test/lint.
- `docs/` — `web-ui.md` (admin-UI screenshots + how to regenerate them) and `spikes/` (investigation notes: the CockroachDB/YugabyteDB datastore evaluation — manifests since removed as non-working, PostgreSQL is the only backend — plus a forward-auth/Traefik DNS-records spike). Screenshots live in `docs/img/`.
...
make e2e-forward-auth           # fast: up -> configure -> follow redirect -> assert Authentik login reachable (HTTP 200) -> down
make e2e-forward-auth-browser   # full: real headless browser signs in -> whoami serves X-authentik-* identity headers (Playwright)
...
- **CI**: `.github/workflows/ci.yml` runs `make static-check` + `make build` + `make test` + `make image-scan` (the `docker` job, **tag-only** — builds the provisioner image and Trivy-scans it for HIGH/CRITICAL, since hadolint only *lints* the Dockerfile) + `make e2e-compose` (the `e2e` job — live Authentik via Compose) + `make e2e` (the `e2e-kind` job — a full KinD cluster + cloud-provider-kind that deploys the real `k8s/postgresql` manifests and provisions/verifies against the LoadBalancer, catching manifest-wiring regressions the Compose e2e can't, ~10-15 min) + `make e2e-forward-auth-browser` (the `e2e-forward-auth-browser` job — a real headless-browser Authentik login against the Traefik/whoami forward-auth overlay, via Playwright; uses node from `.mise.toml`) via `jdx/mise-action` (toolchain from `provisioner/.mise.toml`). **All three e2e jobs are tag-only** (same posture as `docker`/`security`) — they do NOT run on PRs; `publish` gates on their explicit success, so they still block a *release*, not a merge. Consequence to know: `e2e-kind` is the only job that exercises the real k8s manifests against a cluster, so a manifest-wiring regression now merges green and surfaces at tag time — `k8s-drift` still runs per-change but only diffs the helm render, it cannot see a wiring break. To validate before merging, dispatch on the branch: `gh workflow run ci.yml --ref <branch>` (each e2e job carries a `workflow_dispatch` arm for exactly this). The Go project is in `provisioner/`, so jobs set `working-directory: provisioner`. A `dorny/paths-filter` `changes` job gates the heavy jobs on `provisioner/**`/`.github/workflows/**`/`CLAUDE.md`/`compose/**`/`e2e/**`/`.dclintrc.yaml`/`.gitleaks.toml` edits (`compose/**` + `e2e/**` are included so `static-check`'s `compose-lint` runs on the relevant changes; `.gitleaks.toml` because `static-check`'s `secrets` gate consumes it) — pure-k8s changes skip the code path. Note the `code` filter no longer gates the e2e jobs, which are tag-only. A separate `docs` filter (`README.md`) drives a cheap `mermaid-lint` job on README-only edits (the README carries Mermaid diagrams that `static-check` would otherwise skip). A `ci-pass` job aggregates results (`changes`, `static-check`, `build`, `test`, `docker`, `security`, `k8s-drift`, `e2e`, `e2e-kind`, `e2e-forward-auth-browser`, `mermaid-lint`, `publish`). **CVE scanning (`vulncheck` + `trivy-fs`, via `make cve-scan` — the `security` job), the image build/scan (`docker` job), and all three e2e jobs run on version tags ONLY, not on PRs** — so a freshly-disclosed CVE-DB drift can't block unrelated PRs and the heavy live-infra jobs stay off the PR path; the release gate runs them at tag time. The PR path is therefore `static-check` + `build` + `test` (+ `k8s-drift` on k8s changes, `mermaid-lint` on README-only edits). On a `v*` tag a `publish` job builds + pushes the provisioner image to GHCR (`ghcr.io/<owner>/authentik-k8s/provisioner`, authed with the built-in `GITHUB_TOKEN`), gated behind the **explicit success** of `static-check`+`build`+`test`+`docker`+`security`+`e2e`+`e2e-kind`+`e2e-forward-auth-browser` on the tagged commit (`needs.*.result == 'success'`, NOT `!failure()` — a *skipped* job must not let an unscanned image ship). No operator secrets required (only the automatic `GITHUB_TOKEN`).
- The local quality gate (`provisioner/Makefile` → `make ci`: golangci-lint + go-mod-tidy + toolchain-alignment) mirrors the PR-path CI. CVE scanning (`make cve-scan`: govulncheck + trivy-fs) is run by CI on tags only (the `security` job) — run it locally before cutting a release. `.golangci.yml` runs the standard linters; gosec is intentionally omitted (the POC hardcodes test tokens and skips TLS verify by design — documented in `.golangci.yml`). There is intentionally **no `make ci-run` (act) target**: CI is dominated by Compose/KinD/Playwright jobs that `act` cannot run, so `make ci` (`static-check` + `test` + `build`) is the local mirror of the act-runnable surface.
- **Test layers**:
  - *Unit + hermetic httptest contracts* (`make test`, no infra): `internal/authentik` (~83%) — `CreateConfiguration` auth-header contract + httptest contracts for every `CoreApi` wrapper at the real `/api/v3/...` paths, plus the forward-auth wrappers (`forwardauth_httptest_test.go`: flow resolution, ProxyProvider/Application create + idempotency, embedded-outpost find + bind, and the whole `SetupForwardAuth` flow); `internal/util` (87.5%); `main` (~68%) — `CreateGroupsAndUsers` whole-flow + per-step error branches (`main_test.go`), `deriveNames`, and the env-gate/`ForwardAuthConfig` assembly helpers (`env`, `buildProvisionRequests`, `buildForwardAuthConfig` in `main_planning_test.go`, injected via a `func(key,fallback)` lookup so the gate logic is unit-testable); only the thin `main()` wiring + `log.Fatal` paths are uncovered. `CreateGroupsAndUsers` returns `error` (not `log.Panicf`) so the flow is testable.
  ...
  - *Live forward-auth e2e — fast* (`make e2e-forward-auth`, Makefile/shell-driven — not a Go test): brings up the `compose/forward-auth/` overlay, configures the provider via the POC, then **follows the redirect** and asserts the Authentik login is reachable (HTTP 200 + `ak-flow-executor`). Self-tears-down. (Asserting just the 302 would be a fake-green — the redirect can dead-end at an unreachable host; it must reach a real login.)
  - *Live forward-auth e2e — full browser* (`make e2e-forward-auth-browser`, Playwright in `e2e/browser/`): a real headless Chromium completes the actual Authentik login (akadmin) and asserts whoami serves the `X-authentik-*` identity headers — the true end-user outcome. `compose-forward-auth-up` waits until the login is reachable (the embedded outpost's `authentik_host` propagated) before the test, so the gate is honest (`retries: 0`). This is the CI `e2e-forward-auth-browser` job (node from `.mise.toml`; `@playwright/test` pinned in `e2e/browser/`).
...
Deferred / monitor items from `/upgrade-analysis`. Every toolchain/image version is pinned in its source of truth — `provisioner/.mise.toml`, `provisioner/go.mod`, the Dockerfile `FROM`/`ARG`, Makefile `_VERSION` constants, and the compose + `k8s/` `image:` tags — and all are Renovate-tracked (`renovate.json`). Run `/upgrade-analysis` for a freshness check rather than trusting a version snapshot here: enumerated version literals in prose drift silently the moment Renovate bumps the pin, with no gate to catch it. Two standing invariants worth keeping in mind: the KinD node image (`kindest/node`) must stay aligned with the pinned `kind` version, and `govulncheck` runs clean. Renovate `automerge` is **on** (`renovate.json` → `"automerge": true`), so dependency PRs land unattended once `ci-pass` is green — that required check is the only thing gating them. Know what that check does and does not cover: the PR path runs `static-check` + `build` + `test` (+ `k8s-drift`, `mermaid-lint`), while `docker`, `security` (govulncheck + trivy) and all three e2e jobs are **tag-only**. So a bump to anything those tag-only jobs exercise — `kind`/`kubectl`/`kindest/node`, `node`, `trivy`, `govulncheck`, traefik/whoami, `AUTHENTIK_TAG`, `@playwright/test` — is verified on a PR only to the extent that `mise install` could download it. Those land unattended with no machine behind them; dispatch `gh workflow run ci.yml --ref <branch>` when a bump in that set deserves real verification.
