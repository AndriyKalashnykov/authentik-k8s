...
- **CI**: `.github/workflows/ci.yml` runs `make static-check` + `make build` + `make test` + `make image-scan` (the `docker` job, **tag-only** — builds the provisioner image and Trivy-scans it for HIGH/CRITICAL, since hadolint only *lints* the Dockerfile) + `make e2e-compose` (the `e2e` job — live Authentik via Compose) + `make e2e` (the `e2e-kind` job — a full KinD cluster + cloud-provider-kind that deploys the real `k8s/postgresql` manifests and provisions/verifies against the LoadBalancer, catching manifest-wiring regressions the Compose e2e can't; gated on code OR `k8s/**` changes, ~10-15 min) + `make e2e-forward-auth-browser` (the `e2e-forward-auth-browser` job — a real headless-browser Authentik login against the Traefik/whoami forward-auth overlay, via Playwright; uses node from `.mise.toml`) via `jdx/mise-action` (toolchain from `provisioner/.mise.toml`). The Go project is in `provisioner/`, so jobs set `working-directory: provisioner`. A `dorny/paths-filter` `changes` job gates the heavy jobs on `provisioner/**`/`.github/workflows/**`/`CLAUDE.md`/`compose/**`/`e2e/**`/`.dclintrc.yaml`/`.gitleaks.toml` edits (compose + e2e are included so `static-check`'s `compose-lint`, the `e2e` job, and the browser e2e run on the relevant changes; `.gitleaks.toml` because `static-check`'s `secrets` gate consumes it) — pure-k8s changes skip CI. A separate `docs` filter (`README.md`) drives a cheap `mermaid-lint` job on README-only edits (the README carries Mermaid diagrams that `static-check` would otherwise skip). A `ci-pass` job aggregates results (`changes`, `static-check`, `build`, `test`, `docker`, `security`, `k8s-drift`, `e2e`, `e2e-kind`, `e2e-forward-auth-browser`, `mermaid-lint`, `publish`). **CVE scanning (`vulncheck` + `trivy-fs`, via `make cve-scan` — the `security` job) and the image build/scan (`docker` job) run on version tags ONLY, not on PRs** — so a freshly-disclosed CVE-DB drift can't block unrelated PRs; the release gate runs them at tag time. On a `v*` tag a `publish` job builds + pushes the provisioner image to GHCR (`ghcr.io/<owner>/authentik-k8s/provisioner`, authed with the built-in `GITHUB_TOKEN`), gated behind the **explicit success** of `static-check`+`build`+`test`+`docker`+`security` on the tagged commit (`needs.*.result == 'success'`, NOT `!failure()` — a *skipped* job must not let an unscanned image ship). No operator secrets required (only the automatic `GITHUB_TOKEN`).
