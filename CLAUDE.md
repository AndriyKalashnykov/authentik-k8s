...
- `k8s/yugabytedb/` — experimental YugabyteDB backend (single-node `yugabytedb/yugabyte` 2025.2.4.0, YSQL on 5433). **Known non-working but for a DIFFERENT reason than CockroachDB**: YugabyteDB v2025.1+ *does* provide `pg_advisory_lock()` (the CockroachDB blocker is gone — verified), but Authentik 2026.5's Django migrations still abort partway on YugabyteDB transaction-conflict errors (`YB001`), even with `yb_enable_read_committed_isolation=true` (re-confirmed on 2025.2.4.0, 2026-06-27). Frozen + Renovate-ignored like cockroachdb; full analysis + the empirical POC results in `docs/spikes/authentik-cockroachdb-yugabytedb.md`.
...
- `docs/` — `web-ui.md` (admin-UI screenshots + how to regenerate them) and `spikes/` (the CockroachDB/YugabyteDB datastore investigation). Screenshots live in `docs/img/`.
...
- [ ] **CockroachDB backend** (`k8s/cockroachdb/`, frozen) — revisit only when [cockroachdb#169981](https://github.com/cockroachdb/cockroach/issues/169981) (session-scoped `pg_advisory_lock()`) closes in a future 26.x. Until then it stays frozen + Renovate-ignored. *Re-verified 2026-06-27: #169981 still open (no milestone) — no change.*
- [ ] **YugabyteDB backend** (`k8s/yugabytedb/`, frozen experimental) — advisory locks work, but Authentik 2026.5 migrations abort on YB001 transaction conflicts (see `docs/spikes/authentik-cockroachdb-yugabytedb.md`). *Re-tested 2026-06-27 on the newer YugabyteDB **2025.2.4.0** line — same failure (§6b); a point/minor bump does not unblock it.* Revisit only when a YugabyteDB release materially changes DDL-transaction **retry** semantics, or Authentik tolerates YugabyteDB's transaction model — not merely on a newer patch.
